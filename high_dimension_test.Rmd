...
tic()
lapply(1:length(lambda.eff), function(i) {
  fit00 <- JGL(Y=X, penalty="fused", lambda1=lambda.eff[i], lambda2=lambda.eff[1], return.whole.theta=TRUE)
  aic_vec1[i,1] <<- vBIC(X, fit00, thr=0.0001)  # Use <<- to assign to the outer scope
})
toc()

library(pbmcapply)
tic()
MC_results <- pbmclapply(1:length(lambda.eff), 
                         function(i) {
  fit00 <- JGL(Y=X, penalty="fused", lambda1=lambda.eff[i], lambda2=lambda.eff[1], return.whole.theta=TRUE)
  aic_vec1[i,1] <<- vBIC(X, fit00, thr=0.0001)  # Use <<- to assign to the outer scope
...
# 6.33 minutes for 500 features, 1000 per arm (112 seconds when in parallel!!) 
# 48 seconds for 200 features, 1000 per arm
# 1.16 hours for 1000 features, 200 per arm- lam_1 0.356, lam_2 0.01
...
curr_res1 <-  (abs(res_fgl_hd$theta[[1]]) > thresh)*1
...
# 16 minutes for n 1000 p 200
# __ minutes for n 200 p 1000
